...
export async function GET(request: NextRequest) {
  try {
    ...
    if (serviceKey) {
      ...
      const { count, error } = await admin.from("user_subscriptions").select("id", { count: "exact", head: true });
    ...
    const { data: rpcCount, error: rpcError } = await supabase.rpc("admin_count_user_subscriptions");
    ...
    console.error("/api/admin/count/user-subscriptions error", err);
